...
public class PlatformGridView extends LinearLayout implements
	...
	private static final int PAGE_SIZE = 8; // 每页显示9格
	...
	public void afterPlatformListGot() {
		...
		lp.height = (scrW - dp_10 * 2) * lines / 4;
	...
	public void onClick(View v) {
		...
		if (plat != null) {
					...
					|| "GooglePlus".equals(name) || "QQ".equals(name))
			{
				...
				//reqData.put("title",reqData.get("text"));//微信的两个平台均只显示标题
				//reqData.put("text"," ");//文本为一个空格,因为微信的图文分享不支持长文本，所以直接清空。
				//-------------End-------------------------------------------//
	...
	private static class PlatformAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			if (girds[position] == null) {
				...
				if (position == 0) {
					lines = gridPlatforms.length / 4;
					if (gridPlatforms.length % 4 > 0) {
						lines++;
					}
				}
	...
	private static class GridView extends LinearLayout {
		...
		private void init() {
			...
			int scrW = getResources().getDisplayMetrics().widthPixels;
			iconWidth = (scrW - dp_10 * 2) / 4 - dp_10 * 4;
		...
		private LinearLayout getView(int position, Context context) {
			...
			int dp_5 = cn.sharesdk.framework.utils.R.dipToPx(context, 1);
