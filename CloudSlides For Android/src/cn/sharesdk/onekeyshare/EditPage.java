...
public class EditPage extends FakeActivity implements OnClickListener, TextWatcher, Callback {
	...
	private void initPageView() {
		...
		//-----下面这一行为自定义代码-----------------------------
		etContent.setText(String.valueOf("#"+reqData.get("title")+"#  "+reqData.get("text")));
		//-----End---------------------------------------------
	...
	public void onClick(View v) {
		...
		if (v.equals(llTitle.getBtnRight())) {
			...
			if (selected) {
				...
				{
					Toast.makeText(getContext(),"已超出字数上限(140字),请重新编辑!",Toast.LENGTH_LONG).show();
					return;
				}
				else 
					if (parent != null) {//向所有勾选的社交平台分享
	...
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		int remain = 140 - etContent.length()-20;	
		tvCounter.setTextColor(remain > 0 ? 0xffcfcfcf : 0xffff0000);
		if(remain>=0)
		{
			tvCounter.setText("还可以输入"+String.valueOf(remain)+"字");
		}
		else
		{
			tvCounter.setText("输入超出"+String.valueOf(-remain)+"字");
		}
	}
